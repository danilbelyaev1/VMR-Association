<template>
  <div v-click-outside="hide" class="select-default">
    ...
    <p class="select-input" @click="toggle">
      <span v-if="selected.length <= 3">
        <span
          v-for="elem in selectedSort"
          :key="elem"
          @click="selectOption(elem)"
        >
          {{ elem + '%' }}
        </span>
      </span>
      <span v-else>Выбрано {{ selected.length }} из {{ options.length }}</span>
      <svg
...
export default {
  ...
  computed: {
    selectedSort() {
      return [...this.selected].sort(compareNumbers)
    },
  },
  methods: {
    hide() {
