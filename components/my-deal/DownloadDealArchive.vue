<template>
  <a v-if="dealId && canDownloadDocuments" @click.prevent="downloadArchive">
    <svg
      width="20"
      height="24"
      viewBox="0 0 20 24"
    ...
    Скачать файлы подписей
  </a>
</template>

<script>
export default {
  name: 'DownloadDealArchive',
  props: {
    deal: {
      type: Object,
      required: true,
  ...
  computed: {
    ...
    canDownloadDocuments() {
      return this.deal?.contract?.can_download_documents || false
    },
  },
  methods: {
    async downloadArchive() {
      try {
        const url = `/deals/${this.dealId}/sign/download-data`
        ...
        await this.$axios
          ...
          .then((response) => {
            ...
            link.download = `договор и подписи сделки-${this.dealId}.zip`
            link.click()
          })
      } catch (e) {
