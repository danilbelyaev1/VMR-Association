<template>
  <a v-if="dealId && urlDocument" @click.prevent="downloadDocument">
    <svg
      width="20"
      height="24"
      viewBox="0 0 20 24"
    ...
    Скачать акт приема-передачи №{{ dealId }}
  </a>
</template>

<script>
export default {
  name: 'DownloadDealAct',
  props: {
    deal: {
      type: Object,
      required: true,
  ...
  computed: {
    ...
    urlDocument() {
      return this.deal?.act_document?.url || false
    },
  },
  methods: {
    downloadDocument() {
      const link = document.createElement('a')
      link.href = this.urlDocument
      link.download = `Акт сделки #${this.dealId}.docx`
      link.click()
    },
  },
}
