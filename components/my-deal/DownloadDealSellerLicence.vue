<template>
  <div v-if="dealId">
    <a @click.prevent="download">
      ...
      Скачать лицензию продавца
    </a>
    <DefaultModal
      modal-class=""
      close-button-text="Понятно"
      modal-id="downloadSellerLicenceErrorModal"
      :title="error"
    />
  </div>
</template>
...
<script>
export default {
  name: 'DownloadDealSellerLicence',
  props: {
    deal: {
      type: Object,
      required: true,
  ...
  methods: {
    async download() {
      try {
        const url = `/deals/${this.dealId}/download-seller-licence`
        ...
        link.download = `лицензии_продавца_сделки_${this.dealId}.zip`
        link.click()
      } catch (e) {
        ...
        if (errorJson?.message) {
          this.error = errorJson?.message
        }
        this.$bvModal.show('downloadSellerLicenceErrorModal')
      }
    },
  },
}
